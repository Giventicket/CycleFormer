...
class EncoderPositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, T, dropout, mode):
    ...
    def forward(self, embeddings, graph):
        ...
        elif "rad" in self.mode:
            rs = torch.sqrt(xs**2 + ys**2).unsqueeze(-1)
            a_term = rs*self.div_term.repeat(batch_size,node_size,1)
            thetas = torch.atan2(ys, xs).unsqueeze(-1)
            b_term = thetas * self.div_term.repeat(batch_size, node_size, 1)
        ...
        if "cat" in self.mode:
            ...
            # [batch_size, node_size, 128]
        elif "sum" in self.mode:
            pe = (pe_x + pe_y).requires_grad_(False)
...
class DecoderCPE(nn.Module):

    def __init__(self, d_model, dropout, seq_len):
        super(DecoderCPE, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
...
def make_model(src_sz, tgt_sz, enc_num_layers=6, dec_num_layers=6, d_model=128, d_ff=512, h=8, dropout=0.1, mode = "xy_cat", share_lut=False, use_decoderCPE = False):
    ...
    if share_lut:
        src_embed = tgt_embed = Embeddings(d_model)
    else:
    ...
    model = EncoderDecoder(
        ...
        encoder_pe=EncoderPositionalEncoding(d_model, 2, dropout, mode),
        tgt_embed=tgt_embed, # decoder
        decoder_pe=DecoderCPE(d_model, dropout, src_sz) if use_decoderCPE else DecoderPositionalEncoding(d_model, dropout, src_sz),
        generator=Generator(),
    )
