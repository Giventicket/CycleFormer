...
from dataset_inference2 import TSPDataset, collate_fn, make_tgt_mask
...
class TSPModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            ys = tgt.clone()
            visited_mask = visited_mask[:, -1, :].clone().unsqueeze(1)

            for i in range(self.cfg.node_size - ntoken):
                # memory, tgt, tgt_mask
                tgt_mask = subsequent_mask(ys.size(1)).type(torch.bool).to(src.device)
                out = self.model.decode(memory, src, ys, tgt_mask)
                prob = self.model.generator(out[:, -1].unsqueeze(1), visited_mask, self.model.encoder_lut)
